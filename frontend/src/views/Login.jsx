  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-100 p-4 font-sans text-slate-800 antialiased text-xs">
      <div className="bg-white rounded-xl shadow-lg border max-w-5xl w-full grid grid-cols-1 md:grid-cols-2 overflow-hidden">
        ...
        <div className="bg-slate-50 p-5 md:p-6 flex flex-col justify-between border-b md:border-b-0 md:border-r border-slate-200 space-y-4">
          <div className="space-y-3">
            <div className="flex flex-wrap items-center gap-2 text-indigo-600 font-black tracking-wider text-sm uppercase">
              🛡️ SMARTCAMPUS <span className="text-[10px] md:text-xs bg-indigo-50 border border-indigo-100 px-1.5 py-0.5 rounded tracking-wide">{"[SAAS-PRO-4.0]"}</span>
            </div>
            <h2 className="text-lg md:text-xl font-black text-slate-900 tracking-tight">Isolated Router Gateway</h2>
            ...
            <div className="bg-white border border-slate-200 p-4 rounded-xl space-y-2 max-h-[250px] md:max-h-[350px] overflow-y-auto custom-scrollbar font-mono text-[10px] text-slate-500 shadow-inner">
              <div className="flex items-center gap-1.5 font-bold text-indigo-600 border-b pb-1">
                <Info size={12} className="shrink-0"/> Examiner Demo: Isolation Matrix Keys [Manual Reference]
              </div>
              <div className="font-bold text-slate-800 mt-2">🏢 GNIOT Node: Tenant [GNIOT-AI-DS]</div>
              <ul className="list-disc pl-4 space-y-0.5">
                <li className="break-all">Admin: admin@gniot.edu</li>
                <li className="break-all">Faculty: aksingh@gniot.edu</li>
              </ul>
              <div className="border-t pt-2 mt-2 font-bold text-slate-800">🏢 DPS Node: Tenant [DPS-CORE-SAAS]</div>
              <ul className="list-disc pl-4 space-y-0.5">
                <li className="break-all">Admin: admin@dps.edu</li>
                <li className="break-all">Faculty: sharma@dps.edu</li>
              </ul>
            </div>
          </div>
          <div className="text-[9px] md:text-[10px] font-mono text-slate-400 flex items-center gap-1.5 pt-3 md:pt-2 border-t border-slate-200 mt-2">
            <ShieldCheck size={12} className="text-emerald-600 shrink-0"/> Data Shard Access Handshake: 100% Multi-Tenancy Secured.
          </div>
        </div>

        {/* Right Input Form */}
        <form onSubmit={handleLoginSubmit} className="p-5 md:p-8 space-y-4 flex flex-col justify-center">
          <div className="space-y-1">
            <label className="text-[10px] md:text-[11px] font-mono font-bold text-slate-400 uppercase tracking-wide block">{"01 // Access Role"}</label>
          ...
          {role !== 'super-admin' && (
            <div className="space-y-1">
              <label className="text-[10px] md:text-[11px] font-mono font-bold text-slate-400 uppercase tracking-wide block">{"01.B // Target Tenant ID Shard"}</label>
          ...
          <div className="space-y-1">
            <label className="text-[10px] md:text-[11px] font-mono font-bold text-slate-400 uppercase tracking-wide block">{"02 // Identity Mail Vector"}</label>
          ...
          <div className="space-y-1">
            <label className="text-[10px] md:text-[11px] font-mono font-bold text-slate-400 uppercase tracking-wide block">{"03 // Security Token Pass"}</label>
            <input 
              type="text" 
              value={password} 
          ...
          <button type="submit" className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-mono text-[10px] md:text-[11px] font-bold py-3.5 rounded-xl uppercase tracking-widest shadow-md mt-4 transition-colors flex items-center justify-center gap-2">
